...
def create_binary(list_maps, list_habitat_classes, zero = True,
  ...
  for i_in in list_maps:
    ...
    if export == True and dirout != "":
      ...
      grass.run_command("r.out.gdal", input=i+"_HABMAT", out=i+"_HABMAT.tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)
...
def patch_size(input_maps, 
  ...
  for i_in in input_maps:
    ...
    if export == True and dirout != "":
      ...
      grass.run_command("r.out.gdal", input = i+"_patch_AreaHA", out = i+"_patch_AreaHA.tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)
    ...
    if export_pid == True and dirout != "":
      ...
      grass.run_command("r.out.gdal", input = i+"_pid", out = i+"_pid.tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)
  ...
  for i_in in input_maps:
    ...
    for a in list_corridor_width_pixels:
      ...
      if export == True and dirout != "":
        ...
        grass.run_command("r.out.gdal", input = i+"_"+format_escale_name+"m_fragment_AreaHA", output = i+"_"+format_escale_name+"m_fragment_AreaHA.tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)
      # If export_fid == True, the fragment ID map is exported in this folder
      if export_fid == True and dirout != "":
        os.chdir(dirout)
        grass.run_command("g.region", rast = i+"_"+format_escale_name+"m_fid")
        grass.run_command("r.out.gdal", input = i+"_"+format_escale_name+"m_fid", output = i+"_"+format_escale_name+"m_fid.tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)
      if struct_connec and export_struct_connec:
        ...
        grass.run_command("r.out.gdal", input = i+"_"+format_escale_name+"m_structural_connectivity", output = i+"_"+format_escale_name+"m_structural_connectivity.tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)        
...
def percentage(input_maps, scale_list, method = "average", append_name = "",
  ...
  for in_map in input_maps: 
    ...
    for i in scale_list:
      ...
      if export == True and dirout != "":
        os.chdir(dirout)      
        grass.run_command("r.out.gdal", input = outputname, out = outputname+".tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)
  ...
  for i_in in input_maps:
    ...
    for a in list_dilatate_pixels:
      ...
      if export == True and dirout != "" and list_gap_cross[x] != 0:
        ...
        grass.run_command("r.out.gdal", input = i+"_"+format_escale_name+"m_func_connect_AreaHA", output = i+"_"+format_escale_name+"m_func_connect_AreaHA.tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", 
        	overwrite = True)
        
        # If functional_area_complete == True, these maps are also exported
        if functional_area_complete:        
          grass.run_command("r.out.gdal", input = i+"_"+format_escale_name+"m_func_connect_complete_AreaHA", output = i+"_"+format_escale_name+"m_func_connect_complete_AreaHA.tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)
          
        # If functional_connec == True, the functional connectivity maps are also exported
        if functional_connec:
          grass.run_command("r.out.gdal", input = i+"_"+format_escale_name+"m_functional_connectivity", output = i+"_"+format_escale_name+"m_functional_connectivity.tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)
      
      # If export_fid == True, the fragment ID map is exported in this folder
      if export_pid == True and dirout != "" and list_gap_cross[x] != 0:
        os.chdir(dirout)
        grass.run_command("g.region", rast = i+"_"+format_escale_name+"m_func_connect_pid")
        grass.run_command("r.out.gdal", input = i+"_"+format_escale_name+"m_func_connect_pid", output = i+"_"+format_escale_name+"m_func_connect_pid.tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)
        
        # If functional_area_complete == True, these maps are also exported
        if functional_area_complete and list_gap_cross[x] != 0:        
          grass.run_command("r.out.gdal", input = i+"_"+format_escale_name+"m_func_connect_complete_pid", output = i+"_"+format_escale_name+"m_func_connect_complete_pid.tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)
  ...
  for i_in in input_maps:
    ...
    for size in list_corridor_width_pixels:
      ...
      if export == True and dirout != "":
        ...
        grass.run_command("r.out.gdal", input = outputname_meco, output = outputname_meco+".tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True) 
        # Export edge map
        grass.run_command("r.out.gdal", input = outputname_edge, output = outputname_edge+".tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)
        # Export core map
        grass.run_command("r.out.gdal", input = outputname_core, output = outputname_core+".tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)
        
        # If the areas were calculated, export them too
        if calc_edge_core_area:
          # Export edge area clumps
          grass.run_command("r.out.gdal", input = edge_area_mapname, output = edge_area_mapname+".tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)          
          # Export core area clumps
          grass.run_command("r.out.gdal", input = core_area_mapname, output = core_area_mapname+".tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)
          
          # If export_pid == True, export pid maps too
          if export_pid:
            # Export edge pid
            grass.run_command("r.out.gdal", input = edge_pid_mapname, output = edge_pid_mapname+".tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)          
            # Export core pid
            grass.run_command("r.out.gdal", input = core_pid_mapname, output = core_pid_mapname+".tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)            
...
def dist_edge(input_maps,
  ...
  for i_in in input_maps:
    ...
    if export == True and dirout != "":
      os.chdir(dirout)
      grass.run_command("r.out.gdal", input = i+"_edge_dist", out = i+"_EDGE_DIST.tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True)
    ...
    for i in scale_list:
      ...
      if export == True and dirout != "":
        ...
        for map_name in map_names_to_export:
          grass.run_command("r.out.gdal", input = map_name, out = map_name+".tif", createopt="COMPRESS=DEFLATE,TFW=YES,BIGTIFF=TRUE", overwrite = True) 
