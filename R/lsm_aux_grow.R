...
#' @param flags `[character=""]` \cr
#' @param radius `[logical(1)=FALSE]` \cr If `TRUE`, the function treats non-habitat cells as null; if `FALSE`, the function converts non-habitat zero cells to null cells.
#' @param metric `[numeric()]` \cr
#' @param old `[numeric()]` \cr
#' @param new `[numeric()]` \cr
#' @param mapunits `[numeric()]` \cr
#'
#' @example examples/lsm_aux_grow_example.R
#'
#' @name lsm_aux_grow
#' @export
lsm_aux_grow <- function(input,
    ...
    if (!mapunits) {
        kv <- rgrass::execGRASS("g.region", flags = "g", intern = TRUE)
        kv <- strsplit(kv, "=")
    ...
    if (rgrass::execGRASS("g.findfile", element = "cell", file = input, intern = TRUE)[1] == "") {
    ...
    if (!shrink) {
        ...
        rgrass::execGRASS("r.mapcalc", flags = flags, expression = expr)
    ...
    # apply colors
    rgrass::execGRASS("r.colors", map = output, raster = input)

    # add history
    rgrass::execGRASS("r.support", map = output, history = paste("Created by r_grow wrapper"))
    ...
    cleanup <- function(temp_maps) {
        for (map in temp_maps) {
            if (!is.null(map)) {
                try(
                    rgrass::execGRASS("g.remove", flags = c("f", "b"), type = "rast",
