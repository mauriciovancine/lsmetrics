...
lsm_grass_functional_connectivity <- function(input,
  ...
  if(!input_distance_outside %in% files){
    stop("Input distance outside does not exist. Use the lsm_grass_distance() function with the argument `type = 'outside'` to create it.")
  }
  ...
  rgrass::execGRASS(cmd = "r.mapcalc",
                    flags = "overwrite",
                    expression = paste0(input, output, "_outside_distance_under", gap_crossing, "=if(", input_distance_outside, "> 0 && ", input_distance_outside, "<=", gap_crossing/2, ", 1, 0)"))
  ...
  rgrass::execGRASS(cmd = "r.area",
                    flags = "overwrite",
                    input = paste0(input, output, "_confun", gap_crossing, "_pid"),
  ...
  rgrass::execGRASS(cmd = "r.mapcalc",
                    flags = "overwrite",
                    expression = paste0(input, output, "_confun", gap_crossing, "_area_ha_int=if(", input, output, "_confun", gap_crossing, "_area_ha_int < 1, 1, ", input, output, "_confun", gap_crossing, "_area_ha_int)"))
  rgrass::execGRASS(cmd = "r.mapcalc",
                    flags = "overwrite",
